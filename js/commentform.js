...
jQuery(document).ready(function( $ ) {
    ...
    jQuery(".nojs").addClass("js");
    jQuery(".js").removeClass("nojs");
    ...
    jQuery("#subscribes_form .enewletter_widget_submit").click(function(event){
        ...
        if ( jQuery(this).attr('id') == "new_subscribe" ) {
            if ( "" == parent.find("#e_newsletter_email").val() ) {
                ...
                var theObjecht = parent.find("#message");
                var state = theObjecht.attr('aria-hidden') === 'true' ? false : true;
                theObjecht.attr('aria-expanded', !state);
                theObjecht.attr('aria-hidden', state);
                theObjecht.text( email_newsletter_widget_scripts.empty_email ).slideDown();
